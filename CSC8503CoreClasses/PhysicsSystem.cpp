...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		if ((*i).framesLeft == numCollisionFrames) {
			i->a->OnCollisionBegin(i->b, i->point.penetration);
			i->b->OnCollisionBegin(i->a, i->point.penetration);
		}
		...
		if ((*i).framesLeft < 0) {
			i->a->OnCollisionEnd(i->b, i->point.penetration);
			i->b->OnCollisionEnd(i->a, i->point.penetration);
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				else if (info.a->GetPhysicsObject()->GetChannel() == 3 || info.b->GetPhysicsObject()->GetChannel() == 3) {
...
void PhysicsSystem::ProjectionResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (physA->GetResolve() && physB->GetResolve()) {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if ((a.GetType() == 35 && b.GetType() == 39) || (b.GetType() == 35 && a.GetType() == 39) ||
		(a.GetType() == 35 && b.GetType() == 35) ||
		(a.GetType() == 35 && b.GetType() == 26) || (b.GetType() == 26 && a.GetType() == 35) ||
		(a.GetType() == 39 && b.GetType() == 26) || (b.GetType() == 26 && a.GetType() == 39)) {
		return;
	}
...
void PhysicsSystem::NarrowPhase(float dt) {
	for (std::set<CollisionDetection::CollisionInfo>::iterator
		...
		if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
			...
			if (info.a->GetPhysicsObject()->GetChannel() == 2 || info.b->GetPhysicsObject()->GetChannel() == 2) {
				PenaltyResolveCollision(*info.a, *info.b, info.point);
				UpdateAccel(*info.a, dt);
				UpdateAccel(*info.b, dt);
			}
			else if (info.a->GetPhysicsObject()->GetChannel() == 3 || info.b->GetPhysicsObject()->GetChannel() == 3) {
				ProjectionResolveCollision(*info.a, *info.b, info.point);
			}
...
void PhysicsSystem::UpdateAccel(GameObject& i, float dt) {
	PhysicsObject* object = i.GetPhysicsObject();
	float inverseMass = object->GetInverseMass();
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	std::vector<GameObject*>::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	float frameLinearDampling = 1.0f - (0.8 * dt);
	...
	for (auto i = first; i != last; ++i) {
		...
		linearVel = linearVel * frameLinearDampling;
		object->SetLinearVelocity(linearVel);
		...
		float frameAngularDamping = 1.0f - (0.8f * dt);
