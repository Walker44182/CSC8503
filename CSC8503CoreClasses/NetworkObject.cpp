...
bool NetworkObject::ReadDeltaPacket(DeltaPacket &p) {
	...
	if (p.objectID == object.GetNetworkObject()->GetNetworkID()) {
		...
		if (player) {
			int fullScore = lastFullState.score;
			fullScore += p.score;
			player->SetPlayerScore(fullScore);
			player->SetItem(p.hasItem);
			player->SetFinished(p.finish);
			player->GetRenderObject()->SetColour(p.colour);
		}
...
bool NetworkObject::ReadFullPacket(FullPacket &p) {
	...
	if (p.objectID == object.GetNetworkObject()->GetNetworkID()) {
		...
		if (player) {
			player->SetPlayerScore(lastFullState.score);
			player->GetRenderObject()->SetColour(lastFullState.colour);
			player->SetItem(lastFullState.hasItem);
			player->SetFinished(lastFullState.finish);
		}
	}
...
bool NetworkObject::WriteDeltaPacket(GamePacket**p, int stateID) {
	...
	if (player) {
		...
		dp->score = currentScore;
		dp->hasItem = player->GetItem();
		dp->finish = player->HasFinished();
...
bool NetworkObject::WriteFullPacket(GamePacket**p) {
	...
	if (player) {
		fp->fullState.score = player->GetPlayerScore();
		fp->fullState.colour = player->GetRenderObject()->GetColour();
		fp->fullState.hasItem = player->GetItem();
		fp->fullState.finish = player->HasFinished();
	}
